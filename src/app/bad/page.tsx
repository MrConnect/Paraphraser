...
export default function BadPage() {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      const res = await fetchWithTimeout("/api/bad", {}, ctrl.signal);
  ...
  const handleRemove = async (file: BadFile, permanent: boolean) => {
    ...
    try {
      await fetchWithTimeout("/api/bad", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ filePath: file.path, permanent }) });
      await fetchFiles();
    } catch { alert("فشل"); }
  };

  const handleDeleteAll = async () => {
    if (!confirm("حذف كل ملفات Bad نهائياً؟")) return;
    ...
    <main className="min-h-screen p-4 md:p-8" dir="rtl">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-text-primary">🚫 قائمة Bad</h1>
        ...
        {files.length === 0 ? (
          <div className="text-center py-20 bg-surface-raised/50 rounded-2xl border border-border/30">
            <p className="text-text-secondary">لا توجد ملفات في قائمة Bad</p>
          ...
          <div className="space-y-2">
            {visibleItems.map((file) => (
              <div key={file.path} className="bg-surface-raised rounded-xl border border-border p-3 flex items-center gap-3 hover:border-border-hover transition group">
                <div className="relative w-28 h-18 rounded-lg overflow-hidden bg-surface-overlay shrink-0">
                  {file.thumbnail ? (
